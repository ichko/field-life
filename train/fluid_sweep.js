...
const PAGE = {
  readRho: `function readRho(){ const N = G.N, a = new Float32Array(N*N*4); target(G.rho, N, N);
    gl.readPixels(0, 0, N, N, gl.RGBA, gl.FLOAT, a); return a; }`,
  metrics: `function metrics(a, b){
    const n = a.length/4; let sum = 0, sq = 0, pur = 0, act = 0, occ = 0, mx = 0, edge = 0, bad = 0;
    const N = Math.round(Math.sqrt(n));
    const tot = new Float32Array(n);
    for(let i = 0; i < n; i++){ const t = a[4*i] + a[4*i+1] + a[4*i+2] + a[4*i+3]; tot[i] = t; if(!isFinite(t)) bad++; sum += t; }
    const m = sum/n;
    for(let i = 0; i < n; i++){
      const t = tot[i]; sq += (t - m)*(t - m); if(t > mx) mx = t; if(t > 0.2*m) occ++;
      if(t > 1e-9){ let best = 0; for(let c = 0; c < 4; c++) best = Math.max(best, a[4*i+c]); pur += best; }
      if(b) for(let c = 0; c < 4; c++) act += Math.abs(a[4*i+c] - b[4*i+c]);
